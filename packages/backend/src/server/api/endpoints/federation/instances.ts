	...
	constructor(
		...
		super(meta, paramDef, async (ps, me) => {
			...
			if (typeof ps.blocked === 'boolean') {
				...
				if (ps.blocked) {
					query.andWhere(meta.blockedHosts.length === 0 ? '1=0' : 'instance.host NOT IN (:...blocks)', { blocks: meta.blockedHosts });
				} else {
					query.andWhere(meta.blockedHosts.length === 0 ? '1=1' : 'instance.host IN (:...blocks)', { blocks: meta.blockedHosts });
				}
			}
